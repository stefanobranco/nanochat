...
parser.add_argument("--no-save", action="store_true", help="never write checkpoints, including the final one (for preflights)")
parser.add_argument("--save-model-only", action="store_true", help="periodic saves skip optimizer state (halves checkpoint disk; final save keeps it)")
# Output
...
while True:
    ...
    if not args.no_save and (last_step or (step > 0 and step != args.resume_from_step and args.save_every > 0 and step % args.save_every == 0)):
        save_checkpoint(
            ...
            None if (args.save_model_only and not last_step) else optimizer.state_dict(), # optimizer state
