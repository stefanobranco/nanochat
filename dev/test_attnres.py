...
kx = lambda vs: [norm(v) for v in vs]  # keys are the normed sources (cached by the model)
...
check("at init, output == mean of sources",
      torch.allclose(layer(srcs, kx(srcs)), torch.stack(srcs).mean(0), atol=1e-6))
